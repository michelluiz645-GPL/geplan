...
def pg_dashboard():
    ...
    st.markdown("""<style>
    .kpi{background:#fff;border-radius:14px;padding:20px 22px;position:relative;overflow:hidden;
         box-shadow:0 2px 8px rgba(0,0,0,0.07);border:1px solid #F1F5F9;transition:transform .15s,box-shadow .15s}
    .kpi:hover{transform:translateY(-3px);box-shadow:0 8px 24px rgba(0,0,0,0.11)}
    ...
    .kpi-val{font-size:30px;font-weight:900;color:#1E293B;line-height:1;margin-bottom:4px}
    .kpi-lbl{font-size:12px;font-weight:600;color:#94A3B8;text-transform:uppercase;letter-spacing:.8px}
    .kpi-sub{font-size:11px;margin-top:6px;font-weight:500}
    .kpi-bar{position:absolute;bottom:0;left:0;right:0;height:4px;border-radius:0 0 14px 14px}
    </style>""", unsafe_allow_html=True)

    kpis1 = [
        ("&#9707;", "Produtos",     str(len(prods)), "", "#2563EB","rgba(37,99,235,0.08)"),
        ("&#9750;", "Funcionarios", str(ativos),     "", "#0D9488","rgba(13,148,136,0.08)"),
        ("&#8595;", "Entradas",     str(entradas),   "", "#10B981","rgba(16,185,129,0.08)"),
        ("&#8593;", "Saidas",       str(saidas),     "", "#F97316","rgba(249,115,22,0.08)"),
        ("&#8635;", "Devolucoes",   str(devolucoes), "", "#8B5CF6","rgba(139,92,246,0.08)"),
    ]
    cols = st.columns(5)
    for col,(icon,lbl,val,sub,cor,bg) in zip(cols, kpis1):
        with col:
            st.markdown(f"""<div class="kpi">
              <div class="kpi-icon" style="background:{bg};color:{cor};font-size:20px;font-weight:900;font-family:monospace">{icon}</div>
              <div class="kpi-val">{val}</div>
              <div class="kpi-lbl">{lbl}</div>
              <div class="kpi-bar" style="background:{cor}"></div>
            </div>""", unsafe_allow_html=True)

    st.markdown("<div style='margin-top:12px'></div>", unsafe_allow_html=True)

    cor_crit = "#EF4444" if crit else "#10B981"
    bg_crit  = "rgba(239,68,68,0.08)" if crit else "rgba(16,185,129,0.08)"
    sub_crit = f"{len(crit)} abaixo do minimo" if crit else "estoque ok"
    kpis2 = [
        ("R$",  "Valor em Estoque", fmt_preco(valor), "",       "#F59E0B","rgba(245,158,11,0.08)"),
        ("&#9888;","Itens Criticos",str(len(crit)),   sub_crit, cor_crit, bg_crit),
        ("&#9881;","Equipamentos",  str(len(equips)), "",       "#6366F1","rgba(99,102,241,0.08)"),
        ("&#8680;","Entregas Pend.",str(n_pend),
            "aguardando" if n_pend else "em dia",
            "#F97316" if n_pend else "#10B981",
            "rgba(249,115,22,0.08)" if n_pend else "rgba(16,185,129,0.08)"),
    ]
    cols2 = st.columns(4)
    for col,(icon,lbl,val,sub,cor,bg) in zip(cols2, kpis2):
        with col:
            st.markdown(f"""<div class="kpi">
              <div class="kpi-icon" style="background:{bg}"><span>{icon}</span></div>
              <div class="kpi-val">{val}</div>
              <div class="kpi-lbl">{lbl}</div>
              <div class="kpi-sub" style="color:{cor}">{sub}</div>
              <div class="kpi-bar" style="background:{cor}"></div>
            </div>""", unsafe_allow_html=True)
